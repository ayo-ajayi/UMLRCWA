...
function [layer] = Disk(epsOut, epsIn, r, Nsp, sys)
    %creates a circular disk or cylinder layer

    % r is the radius of the disk (same unit as Lam from SystemSetup)
    % It defines the disk boundary
    % Inside the circle whose epsilon corresponds to `epsIn`
    % Outside the circle with epsilon `epsOut`
    ...
    % │                         │
    % └─────────────────────────┘
    % Nsp= [Nx, Ny]: FFT spatial grid resolution (typically powers of 2, e.g 1024x2024)
    %sys: system structure from SystemSetup
    % Higer Nsp has better accuracy but slower computation

    A = epsDisk(r, epsIn, epsOut, Nsp, sys.Lam);
    [epsM, epsInvM] = rcwa.convmat(A, sys.maxM);
    NV = NormalVectorField_Radial(r, sys.Lam, Nsp, sys.maxM);
    [F, kz] = Dispersion2D(epsM, epsInvM, sys, NV);
    layer = struct('F', {F}, 'kz', kz, 'A', A);
end

function [a] = epsDisk(r, epsIn, epsOut, N, Lam)
    ...
    ind = (xArr.^2 + yArr.^2 <= r.^2);
    a(ind) = epsIn;
end

function NV = NormalVectorField_Radial(R, Lam, Nsp, maxM)
    ...
    Nxx = rcwa.convmat(xA .* xA, maxM);
    Nyy = rcwa.convmat(yA .* yA, maxM);
    Nxy = rcwa.convmat(xA .* yA, maxM);
    Nyx = rcwa.convmat(yA .* xA, maxM);
