...
function [layer] = GratingY(epsOut, epsIn, len, sys)
    [epsM, epsInvM] = GratingY2D([epsOut, epsIn], len, sys.Lam, sys.maxM);
    NV = NormalVectorField_GratingY2D(sys.maxM);
    [F, kz] = Dispersion2D(epsM, epsInvM, sys, NV);
    layer = struct('F', {F}, 'kz', kz);
end

function [epsMat, epsInvMat] = GratingY2D(epsi, len1, Lam, maxM)
    ...
    tempY = 2 * (eps1 - eps2) * sin(len1 * q0 * yInd / 2) ./ (q0 * yInd) / Lam(2);
    tempY(yInd == 0) = (eps1 * len1 + eps2 * (Lam(2) - len1)) / Lam(2);
    tempX(xInd == 0) = 1;
    epsMat = tempX .* tempY;
    ...
    tempY = 2 * (1 / eps1 - 1 / eps2) * sin(len1 * q0 * yInd / 2) ./ (q0 * yInd) / Lam(2);
    tempY(yInd == 0) = (1 / eps1 * len1 + 1 / eps2 * (Lam(2) - len1)) / Lam(2);
    tempX(xInd == 0) = 1;
    epsInvMat = tempX .* tempY;
end

function NV = NormalVectorField_GratingY2D(maxM)
